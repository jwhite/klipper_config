...
##################
    ...
    def _home_to_extruder(self, length, step):
        ...
        self.gear_stepper.do_set_position(0.)
        self._log_trace("Step: %1.fmm" % step)
        self._log_trace("Length: %1.fmm" % length)
        for i in range (int(length / step)):
            pre_move_position = self._counter.get_distance()
            self._log_trace("Moving : %1.fmm" % (step*(i+1) ) )
            self._log_trace(step*(i+1))
            self.gear_stepper.do_move(step*(i+1), 5, self.gear_stepper_accel)
            self.toolhead.dwell(0.2)
            self.toolhead.wait_moves()
            post_move_position = self._counter.get_distance()
            delta = post_move_position - pre_move_position
            self._log_trace("Step #%d: pos: %.1f, last delta: %.1f" % (i, post_move_position, delta))
            self._log_trace("Checking if delta is < half a step")
            if delta < (step - 1) / 2.:
    ...
    def _load_to_nozzle(self):
        ...
        # moved length detection needs to take extruder to nozzle distance into account
        if moved_length < self.end_of_bowden_to_nozzle - 15.:

             # correction attempt to unload the filament
            delta_length = self.end_of_bowden_to_nozzle - moved_length
            for i in range(3):
                self._log_debug("Attempting a toolhead correction move of %.1f" % delta_length)
                ...
                moved_this_time = self._counter.get_distance() - start_distance
                self._log_debug("Moved %.1f" % moved_this_time)
                moved_length += self._counter.get_distance() - start_distance
                self._log_debug("Total moved length %.1f" % moved_length)

                self._log_info("Correction load move done:  %.1f / %.1f, remaining %.1f" % (moved_this_time,delta_length, self.end_of_bowden_to_nozzle - moved_length) )

                delta_length = self.end_of_bowden_to_nozzle - moved_length
                if (delta_length <= 15):
                    self._log_info("Nozzle distance less than 15, close enough.")
                    break
